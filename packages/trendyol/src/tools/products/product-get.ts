...
const inputSchema = {
  ...
  productMainId: z.string().optional().describe('Trendyol productMainId (use this OR barcode, not both).'),
};
...
export function register(server: McpServer, adapter: MarketplaceAdapter): void {
  server.registerTool(
    'product_get',
    {
      ...
      description:
        'Fetch full product detail by barcode or productMainId. ' +
    ...
    async (args) => {
      try {
        ...
        return jsonResult(product, `Ürün: ${product.title} (stok: ${product.quantity}, fiyat: ${product.salePrice} ${'TRY'}).`);
      } catch (err) {
